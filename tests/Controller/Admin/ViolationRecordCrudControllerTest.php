...
class ViolationRecordCrudControllerTest extends TestCase
{
    ...
    {
        // Filters是final类，不能mock，只测试方法存在性
    ...
    {
        // Actions是final类，不能mock，只测试方法存在性
     ...
     */
    private function createViolationRecord(int $id, ViolationType $violationType): ViolationRecord
    {
        $violationRecord = new ViolationRecord();
        $violationRecord->setViolationType($violationType);
        $violationRecord->setViolationContent('测试违规内容');
        $violationRecord->setViolationTime(new \DateTimeImmutable());
        $violationRecord->setProcessedBy('test_admin');
        $violationRecord->setProcessResult('测试处理结果');
        
        // 使用反射设置ID
        $reflection = new \ReflectionClass($violationRecord);
        $idProperty = $reflection->getProperty('id');
        $idProperty->setAccessible(true);
        $idProperty->setValue($violationRecord, $id);
